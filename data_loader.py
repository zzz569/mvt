...
def process_movielens(root_path):
    ...
    # delete the positive edges in eval/test data in the original graph
    train_pos = np.nonzero(train_data[:, 2]) #找到数组train_data中第3列（索引为2）中非零元素的索引。
    train_pos_idx = train_pos[0]  #训练集中非零用户的索引 原始本来就有边的用户的索引
    user_movie_src_processed = train_data[train_pos_idx, 0]  #原始边的源点 边为1的用户的索引 #使用索引数组train_pos_idx从数组train_data中选取特定行，并选择第1列（索引为0）的元素。
    user_movie_dst_processed = train_data[train_pos_idx, 1] #边为1的物品的索引
    edges_dict = {
        ("movie", "mg", "genre"): (movie_genre_src, movie_genre_dst),
        ("genre", "gm", "movie"): (movie_genre_dst, movie_genre_src),
        ("user", "um", "movie"): (
