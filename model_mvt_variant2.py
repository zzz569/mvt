...
# # 输入数据
# input1 = torch.randn(256, 200)
# input2 = torch.randn(256, 200)
# input3 = torch.randn(256, 200)
#
# # 使用注意力机制
# hidden_size = 200
# inputs = [input1, input2, input3]
# attention = Attention(hidden_size)
# output, attn_weights = attention(inputs)
...
class CDAE(nn.Module):
    ...
    def forward(self, batch_user, batch_user_x, batch_user_y):
        ...
        preds_x2y = self.decoder_y(mapped_z_x_user)
        preds_y2x = self.decoder_x(mapped_z_y_user)

        z_x_reg_loss = torch.norm(z_x - mapped_z_x_user, p=1, dim=1)
        z_y_reg_loss = torch.norm(z_y - mapped_z_y_user, p=1, dim=1)
