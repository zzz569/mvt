...
class Dataset(object):
    ...
    def load_data(self, dataset):
        print('loading data...{}')
        peo2movie = pickle.load(open(os.path.join('processed_data', dataset,'peo2movie_id.pkl'), 'rb')) # dict:16367
        peo2book = pickle.load(open(os.path.join('processed_data', dataset,'peo2book_id.pkl'), 'rb')) # dict:16367
        movie2peo = pickle.load(open(os.path.join('processed_data', dataset,'movie2peo_id.pkl'), 'rb')) #dict:18467
        book2peo = pickle.load(open(os.path.join('processed_data', dataset,'book2peo_id.pkl'), 'rb')) #dict:23988
    ...
    def get_part_train_indices(self, domain, percent):
        ...
        row = np.array(row)  #list:203175 --> ndarray:(203175,)
        col = np.array(col)   #list:203175 --> ndarray:(203175,)
