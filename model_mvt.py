...
class Attention(nn.Module):
    ...
    def forward(self, inputs):
        encoder_outputs = torch.stack(inputs, dim=1)
        batch_size, seq_len, _ = encoder_outputs.size()
        attn_scores = torch.matmul(encoder_outputs, self.attn_weights)
        attn_scores = attn_scores.squeeze(-1)
        attn_weights = torch.softmax(attn_scores, dim=1)
        attn_vectors = torch.bmm(attn_weights.unsqueeze(1), encoder_outputs).squeeze(1)
...
class CDAE(nn.Module):
    def __init__(self, NUM_USER, NUM_MOVIE, NUM_BOOK,  EMBED_SIZE, dropout, meta_dim, is_sparse=False):
        ...
        self.item_x_embeddings = nn.Embedding(self.NUM_MOVIE,EMBED_SIZE,sparse= is_sparse)
        self.item_y_embeddings = nn.Embedding(self.NUM_BOOK,EMBED_SIZE,sparse= is_sparse)
    ...
    def forward(self, batch_user, batch_user_x, batch_user_y):
        ...
        batch_user_x_pos_item = get_index(batch_user_x).cuda()
        batch_user_y_pos_item = get_index(batch_user_y).cuda()
        ufea_x = self.item_x_embeddings(batch_user_x_pos_item)
        ufea_y = self.item_y_embeddings(batch_user_y_pos_item)


        #direct
        mapped_x = self.meta_net.forward(ufea_x , batch_user_x_pos_item).view(-1, self.emb_size, self.emb_size)
        mapped_y = self.meta_net.forward(ufea_y , batch_user_y_pos_item).view(-1, self.emb_size, self.emb_size)
        mapped_z_x_user = torch.bmm(z_x.unsqueeze(1), mapped_x).squeeze()
        mapped_z_y_user = torch.bmm(z_y.unsqueeze(1), mapped_y).squeeze()
        ...
        mapped_y_rela = self.meta_net.forward(ufea_x, batch_user_y_pos_item).view(-1, self.emb_size,
                                                                             self.emb_size)
        mapped_z_x_rela = torch.bmm(h_user_x_rela.unsqueeze(1), mapped_x_rela).squeeze()
        mapped_z_y_rela = torch.bmm(h_user_y_rela.unsqueeze(1), mapped_y_rela).squeeze()
        ...
        mapped_y_path = self.meta_net.forward(ufea_y, batch_user_y_pos_item).view(-1, self.emb_size,
                                                                                  self.emb_size)
        mapped_z_x_path = torch.bmm(h_user_x_path.unsqueeze(1), mapped_x_path).squeeze()
        mapped_z_y_path = torch.bmm(h_user_y_path.unsqueeze(1), mapped_y_path).squeeze()

        inputs_x = [mapped_z_x_user,mapped_z_x_rela,mapped_z_x_path]
        inputs_y = [mapped_z_y_user,mapped_z_y_rela,mapped_z_y_path]

        mapped_z_x,_ = self.attention(inputs_x)
        mapped_z_y,_ = self.attention(inputs_y)
