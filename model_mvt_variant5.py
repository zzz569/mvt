...
class CDAE(nn.Module):
    ...
    def forward(self, batch_user, batch_user_x, batch_user_y):
        ...
        batch_user_x_pos_item = get_index(batch_user_x).cuda()  #[256,20]
        batch_user_y_pos_item = get_index(batch_user_y).cuda()  #[256,20]
        ufea_x = self.item_x_embeddings(batch_user_x_pos_item) #[256,20,200]
        ufea_y = self.item_y_embeddings(batch_user_y_pos_item) #[256,20,200]

        mapped_x = self.meta_net.forward(ufea_x , batch_user_x_pos_item).view(-1, self.emb_size, self.emb_size)  #[256,200,200]
        mapped_y = self.meta_net.forward(ufea_y , batch_user_y_pos_item).view(-1, self.emb_size, self.emb_size)  #[256,200,200]
        mapped_z_x_user = torch.bmm(z_x.unsqueeze(1), mapped_x).squeeze()  #[256,200]
        mapped_z_y_user = torch.bmm(z_y.unsqueeze(1), mapped_y).squeeze() #[256,200]

        #关系转换
        ufea_x_rela = self.src_pretrain_model.item_rela_emb_embedding(batch_user_x_pos_item)  # [256,20,200]
        ufea_y_rela = self.tgt_pretrain_model.item_rela_emb_embedding(batch_user_y_pos_item)  # [256,20,200]
        mapped_x_rela = self.meta_net.forward(ufea_x, batch_user_x_pos_item).view(-1, self.emb_size,
                                                                             self.emb_size)  # [256,200,200]
        mapped_y_rela = self.meta_net.forward(ufea_x, batch_user_y_pos_item).view(-1, self.emb_size,
                                                                             self.emb_size)  # [256,200,200]
        mapped_z_x_rela = torch.bmm(h_user_x_rela.unsqueeze(1), mapped_x_rela).squeeze()  # [256,200]
        mapped_z_y_rela = torch.bmm(h_user_y_rela.unsqueeze(1), mapped_y_rela).squeeze()  # [256,200]

        #语义转换
        ufea_x_path = self.src_pretrain_model.item_path_emb_embedding(batch_user_x_pos_item)  # [256,20,200]
        ufea_y_path = self.tgt_pretrain_model.item_path_emb_embedding(batch_user_y_pos_item)  # [256,20,200]
        mapped_x_path = self.meta_net.forward(ufea_x, batch_user_x_pos_item).view(-1, self.emb_size,
                                                                                  self.emb_size)  # [256,200,200]
        mapped_y_path = self.meta_net.forward(ufea_y, batch_user_y_pos_item).view(-1, self.emb_size,
                                                                                  self.emb_size)  # [256,200,200]
        mapped_z_x_path = torch.bmm(h_user_x_path.unsqueeze(1), mapped_x_path).squeeze()  # [256,200]
        mapped_z_y_path = torch.bmm(h_user_y_path.unsqueeze(1), mapped_y_path).squeeze()  # [256,200]

        stacked_tensor_x = torch.stack([mapped_z_x_user, mapped_z_x_rela, mapped_z_x_path])
        stacked_tensor_y = torch.stack([mapped_z_y_user, mapped_z_y_rela, mapped_z_y_path])

        mapped_z_x = torch.mean(stacked_tensor_x, dim=0)
        mapped_z_y = torch.mean(stacked_tensor_y, dim=0)
